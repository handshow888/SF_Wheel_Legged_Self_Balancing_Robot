...
typedef struct kf_t
{
    ...
    Matrix<Xsize, 1> xhat;      // x(k|k)
    Matrix<Xsize, 1> xhatminus; // x(k|k-1)
    Matrix<Usize, 1> u;         // control vector u
    Matrix<Zsize, 1> z;         // measurement vector z
    Matrix<Xsize, Xsize> P;         // covariance matrix P(k|k)
    Matrix<Xsize, Xsize> Pminus;    // covariance matrix P(k|k-1)
    Matrix<Xsize, Xsize> F, FT;     // state transition matrix F FT
    Matrix<Xsize, Usize> B;         // control matrix B
    Matrix<Zsize, Xsize> H;     // measurement matrix H
    Matrix<Xsize, Zsize> HT;
    Matrix<Xsize, Xsize> Q;         // process noise covariance matrix Q
    Matrix<Zsize, Zsize> R;         // measurement noise covariance matrix R
    Matrix<Xsize, Zsize> K;         // kalman gain  K
    Matrix<Zsize, Zsize> S;
